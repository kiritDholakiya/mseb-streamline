...
if ( ! class_exists( 'MSBE_Streamline' ) ) {
	...
	class MSBE_Streamline {
		...
		public function __construct() {
			...
			add_action(
				'sync_streamline_featured_properties',
				array(
					$this,
			...
			add_action(
				'sync_streamline_inactive_properties',
				array(
					$this,
			...
			// add coupon box
			add_action(
				'checkout_properties_price_before',
				array(
					$this,
			...
			add_filter(
				'vrb_check_' . $this->vrb_engine . '_property_availability',
				array(
					$this,
			...
			add_action(
				'wp_ajax_nopriv_vrb_process_checkout_stripe_key',
				array(
					$this,
		...
		public function streamline_enqueue_admin_scripts() {
			...
			wp_register_script( 'streamline-backend', GDVRB_STREAMLINE_URL . 'assets/js/streamline-backend.js', array( 'jquery' ), time() );

			// Localize the script with AJAX parameters for Admin
			wp_localize_script(
				'streamline-backend',
				'ajax_streamline_params',
				array(
					'home_url'   => home_url(),
		...
		public function sync_single_property_data() {
			if ( ! isset( $_POST['security'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['security'] ) ), 'script_js_nonce' ) || ! current_user_can( 'manage_options' ) ) {
				wp_send_json_error( array( 'message' => 'Invalid nonce or insufficient permissions.' ), 403 );
			}

			$property_post_id = isset( $_POST['post_id'] ) ? absint( wp_unslash( $_POST['post_id'] ) ) : null;
			$property_id      = isset( $_POST['property_id'] ) ? sanitize_text_field( wp_unslash( $_POST['property_id'] ) ) : '';
			$property_engine  = isset( $_POST['engine'] ) ? sanitize_text_field( wp_unslash( $_POST['engine'] ) ) : '';
		...
		public function set_property( $property, $post_id = 0 ) {
			...
			if ( isset( $property['unit_amenities'] ) && ! empty( $property['unit_amenities'] ) ) {
				foreach ( $property['unit_amenities']['amenity'] as $key => $amenities ) {
					...
					if ( $amenities['amenity_show_on_website'] == 'yes' ) {
						...
						$excluded_categories = apply_filters( 'custom_excluded_amenities', array() );
						...
						if ( term_exists( $amenities['amenity_name'], 'properties_amenities' ) === null ) { // array is returned if taxonomy is given
							$amenities_category = wp_insert_term(
			...
			$property_meta = apply_filters(
				'vrb_property_data_save',
				array(
					'post_data' => array(
		...
		public function sync_property_review( $property_id, $post_id ) {
			...
			$startdate = gmdate( 'Y-m-d', strtotime( "-{$days_to_fetch} days" ) );
			...
			// Maps
			$existing_review_map         = array();
			$existing_comment_signatures = array();

			foreach ( $existingComments as $c ) {
				$stored_review_id = get_comment_meta( $c->comment_ID, 'streamline_review_id', true );

				if ( $stored_review_id !== '' ) {
					$existing_review_map[ (string) $stored_review_id ] = array(
				...
				foreach ( $feedbacks as $review ) {
					...
					if ( isset( $existing_review_map[ (string) $review_id ] ) ) {
						...
						if ( $stored_content !== $new_content ) {
							...
							unset(
								$existing_comment_signatures[ strtolower( trim( $author_name ) ) . '|' . strtolower( trim( $existing_entry['content'] ) ) ]
							);
					...
					$comment_data = array(
						...
						'comment_date'         => ! empty( $review['creation_date'] )
								? gmdate( 'Y-m-d H:i:s', strtotime( $review['creation_date'] ) )
								: current_time( 'mysql' ),
						'comment_approved'     => 1,
					);
					...
					if ( $comment_id ) {
						...
						if ( ! empty( $review['enddate'] ) ) {
							add_comment_meta( $comment_id, 'vrb_rating_date', gmdate( 'Y-m-d', strtotime( $review['enddate'] ) ) );
						}

						$existing_review_map[ (string) $review_id ] = array(
		...
		public function set_property_availability( $property_id, $post_id ) {
			...
			$end_date   = gmdate( 'm/d/Y', strtotime( '+1 year' ) );
			...
			try {
				$result = $wpdb->query( $wpdb->prepare( "DELETE FROM {$propertyAvailabilityTbl} WHERE property_id = %d", $post_id ) ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- table name is a trusted $wpdb property, not user input.
				...
				if ( isset( $response['data'] ) && $response['data'] != '' ) {
					...
					foreach ( $entries as $entry ) {
						$wpdb->insert( $propertyAvailabilityTbl, $entry, array( '%d', '%s', '%s', '%s', '%d', '%d', '%d' ) );
					}
		...
		public function get_discount_pricing( $property_post_id, $unit_post_id, $output_html = true ) {
			$check_in  = isset( $_REQUEST['check_in'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['check_in'] ) ) : '';
			$check_out = isset( $_REQUEST['check_out'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['check_out'] ) ) : '';
			if ( $check_in != '' && $check_out != '' ) {
				$check_in  = gmdate( 'Y-m-d', strtotime( $check_in ) );
				$check_out = gmdate( 'Y-m-d', strtotime( $check_out ) );
			} else {
				$check_in  = gmdate( 'Y-m-d' );
		...
		public function sync_streamline_featured_properties_function() {
			...
			if ( isset( $response['data'] ) && ! empty( $response['data'] ) ) {
				foreach ( $response['data']['property'] as $propertykey => $property ) {
					...
					$start_date      = gmdate( 'm/d/Y' );
					$end_date        = gmdate( 'm/d/Y', strtotime( '60 days' ) );
					$params          = array(
						'startdate' => $start_date,
						'enddate'   => $end_date,
						'unit_id'   => $property['id'],
		...
		public function streamline_enqueue_scripts() {
			global $vrb_options;
			wp_register_script( 'streamline-js', GDVRB_STREAMLINE_URL . 'assets/js/streamline.js', array( 'jquery' ), time() );
		...
		public function applying_coupon() {
			...
			$num_adult   = isset( $_POST['num_adult'] ) ? absint( wp_unslash( $_POST['num_adult'] ) ) : 0;
			$num_child   = isset( $_POST['num_child'] ) ? absint( wp_unslash( $_POST['num_child'] ) ) : 0;
			$property_id = isset( $_POST['property_id'] ) ? sanitize_text_field( wp_unslash( $_POST['property_id'] ) ) : '';
		...
		public function rbn_save_meta_box( $post_id ) {
			if ( array_key_exists( 'rbn_value', $_POST ) ) {
				update_post_meta( $post_id, 'rbn_display_callback', sanitize_text_field( wp_unslash( $_POST['rbn_value'] ) ) );
			}
		}
			...
			if ( $import_single_properties ) {
				...
				<?php if ( ! empty( $last_sync_date ) ) { ?>
					<div class="last-update-text">
						<div>Last Sync : <?php echo gmdate( 'm/d/Y h:i:s a', strtotime( $last_sync_date ) ); ?></div>
					</div>
					<?php
				}
			} else {
		...
		public function get_property_unit_ID( $post_id ) {
			...
			$unit_id      = $wpdb->get_row( $wpdb->prepare( "SELECT post_id FROM {$wpdb->postmeta} WHERE meta_key = 'unit_id' AND meta_value = %s", $post_id ), ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- table name is a trusted $wpdb property, not user input.
			$unit_post_id = '';
		...
		public function streamline_property_check_availability( $engine_response, $args ) {
			...
			// phpcs:disable WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.PreparedSQL.NotPrepared -- table name is a trusted $wpdb property, not user input; $query is already the result of $wpdb->prepare().
			...
			if ( isset( $response['data'] ) && ! empty( $response['data'] ) ) {
				...
				if ( ! empty( $data['taxes_details'] ) ) {
					...
					} elseif ( ! empty( $data['taxes_details']['value'] ) ) {
				...
				if ( ! empty( $data['required_fees'] ) ) {
					...
					} elseif ( ! empty( $data['required_fees']['value'] ) ) {
		...
		public function process_checkout_stripe_key() {
			...
			if ( ! isset( $_POST['security'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['security'] ) ), 'vrb_checkout_nonce' ) ) {
			...
			if ( $stripe_key ) {
				...
				wp_send_json_error( array( 'message' => 'Stripe publishable key not found.' ), 404 );
			}
		...
		public function save_streamline_api_key() {
			...
			if ( isset( $type ) && $type == 'stripe-key' ) {
				$stripe_publishable_key = isset( $_POST['publishable_key'] ) ? sanitize_text_field( wp_unslash( $_POST['publishable_key'] ) ) : '';
